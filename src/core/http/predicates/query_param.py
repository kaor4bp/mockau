...
if __name__ == '__main__':
    ...
    p2 = QueryParamItems(
        query_params=GenericArrayEqualTo[QueryParamItem](value=[QueryParamItem(key='foo', value='bar')])
    )
    print(p1.is_subset_of(p2.compile_predicate()))
    print(p1.is_superset_of(p2.compile_predicate()))
    print(p1.is_intersected_with(p2.compile_predicate()))
