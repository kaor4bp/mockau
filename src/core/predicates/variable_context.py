...
class EvaluateValueMixin(BaseVariableContext):
    def __init__(self, level=None, prefix: str = 'var') -> None:
        super().__init__(level=level, prefix=prefix)
        self._key_vars = []
    ...
    def evaluate_value(self, solver: z3.Solver):
        ...
        return calculated_values[var_id]


class VariableContext(EvaluateValueMixin, BaseVariableContext):
    ...
    def to_global_constraints(self) -> z3.BoolRef:
        ...
        for array_var in self._arrays_with_unbounded_length:
            array_limitations.append(z3.Length(array_var) <= self._get_unbounded_array_len())

        nested_context_constraints = [child.to_global_constraints() for child in self._children]
        return z3.And(type_union_expression, *self._global_constraints, *array_limitations, *nested_context_constraints)
