...
class JsonDatatype:
    ...
    _Z3_JSON_DATATYPES = {}
    _Z3_OBJECT_DATATYPES = {}
    MAX_NESTING_LEVEL = 100

    def __init__(self, level: int | None = None, prefix: str = 'json_var') -> None:
        ...
        """
        if not self._Z3_JSON_DATATYPES:
            self._build_json_type(level=self.MAX_NESTING_LEVEL)
        if level is None:
            self._level = max(self._Z3_JSON_DATATYPES.keys())
        else:
            self._level = level

        variable_id = uuid4()
        self._var_id = variable_id
        self._var = z3.Const(f'{prefix}_{self._var_id}', self.JsonType)

    @property
    def JsonType(self):
        return self._Z3_JSON_DATATYPES[self._level]

    @property
    def ChildJsonType(self):
        return self._Z3_JSON_DATATYPES[self._level - 1]

    @property
    def ObjectItemType(self):
        return self._Z3_OBJECT_DATATYPES[self._level]
    ...
    def _build_json_type(cls, level: int = -1):
        ...
        if level > 0:
            ...
            json_datatype.declare(
                'object',
                ('get_object', z3.ArraySort(z3.StringSort(), nested_json_datatype)),
            )

            json_list = z3.SeqSort(nested_json_datatype)
            json_datatype.declare(
                'array',
                ('get_array', json_list),
            )
        ...
        cls._Z3_JSON_DATATYPES[level] = json_datatype
...
class VariableContext:
    ...
    def __init__(self, level=None, prefix: str = 'var') -> None:
        ...
        self._default_array_len = 10

    @property
    def default_array_len(self):
        return max([c.default_array_len for c in self._children] + [self._default_array_len])
    ...
    def create_child_context(self, prefix: str = 'var') -> 'VariableContext':
        ...
        .. Docstring generated by Gemini 2.5 Flash, modified by Gemini 2.5 Flash
        """
        new_child_context = VariableContext(level=self.json_type_variable.level - 1, prefix=prefix)
    ...
    def create_sibling_context(self, prefix: str = 'var') -> 'VariableContext':
        new_sibling_cntext = VariableContext(level=self.json_type_variable.level, prefix=prefix)
        self._siblings.append(new_sibling_cntext)
        return new_sibling_cntext
    ...
    def to_global_constraints(self) -> z3.BoolRef:
        ...
        nested_context_constraints += [sibling.to_global_constraints() for sibling in self._siblings]
        return z3.And(type_union_expression, *self._global_constraints, *nested_context_constraints)
